...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const currentActiveTab: ActiveTab = activeTab || currentTab || 'hierarchy';
  const handleTabSelect = onTabChange || onSelectTab || (() => {});
  const alertsBadgeCount = unreadAlertsCount ?? activeAlertCount ?? 0;
  ...
  const activeUser = currentUser || {
    ...
    email: 'admin@org.com',
    organizationId: currentOrg?.id || 'default',
  };
  ...
  const navItems: { id: ActiveTab; label: string; icon: React.ReactNode; badge?: string | number; badgeColor?: string }[] = [
    ...
    },
    {
      ...
      badge: alertsBadgeCount > 0 ? alertsBadgeCount : undefined,
  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0 min-h-[calc(100vh-61px)]">
      {/* Scope summary card for current logged in user */}
      <div className="p-3.5 border-b border-slate-800/80 bg-slate-950/40 m-2 rounded-xl">
        ...
        <div className="mt-2.5 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-400">
          ...
          <span className="font-semibold text-slate-200">
            {activeUser.role === 'dg' ? '🌟 Global (Toute l’Entité)' : 
             activeUser.role === 'chef_departement' ? '🏢 Département' :
             activeUser.role === 'directeur' ? '📂 Direction' :
             activeUser.role === 'chef_division' ? '📁 Division' :
             activeUser.role === 'chef_service' ? '📋 Service' : '⚙️ Exécutant Service'}
          </span>
        </div>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                {item.badge !== undefined && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded border ${item.badgeColor || 'bg-slate-800 text-slate-300 border-slate-700'}`}>
                    {item.badge}
                  </span>
                )}
